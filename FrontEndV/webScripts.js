...
function checkRegisterCredentials()
{
	...
	{
        	sendRegisterCredentials(firstname, lastname, username, email, phone, password);
    	}
	else
	{
...
function sendRegisterCredentials(firstname, lastname, username, email, phone, password)
{
    	var httpReq = createRequestObject();
    	httpReq.onreadystatechange = function()
	{
    	...
    	httpReq.open("GET", "webCases.php?type=RegisterNewUser&username=" + username + "&password=" + password + "&firstname=" + firstname + "&lastname=" + lastname + "&email=" + email + "&phone=" + phone);
    	httpReq.send(null);
}
